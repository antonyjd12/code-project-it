...
function useDemoCredentials() {
    document.getElementById('loginEmail').value = 'demo@jdrestaurant.com';
...
// ========================================
// Override the existing handleLogin with better error messages
...
window.handleLogin = function(event) {
    ...
    if (!email) {
        showError('loginEmailError', '⚠️ Email address is required');
        isValid = false;
    } else if (!isValidEmail(email)) {
        showError('loginEmailError', '⚠️ Please enter a valid email format (e.g., user@example.com)');
        isValid = false;
    }

    if (!password) {
        showError('loginPasswordError', '⚠️ Password is required');
        isValid = false;
    } else if (password.length < 6) {
        showError('loginPasswordError', '⚠️ Password must be at least 6 characters');
        isValid = false;
    }
    ...
    setTimeout(() => {
        ...
        if (user) {
            ...
            showSuccessModal(
                'Welcome Back!', 
                `Hello ${user.fullName}! You are now logged in. Redirecting to your dashboard...`
            );
            ...
            const emailExists = usersDatabase.find(u => u.email === email);
            ...
            if (emailExists) {
                ...
                showErrorModal(
                    'Account Not Found', 
                    `No account found with email "${email}". Please check your email or create a new account.`
                );
            }
        }
    }, 1500);
...
window.handleRegister = function(event) {
    ...
    if (!fullName) {
        showError('fullNameError', '⚠️ Full name is required');
        isValid = false;
    } else if (fullName.length < 3) {
        showError('fullNameError', '⚠️ Full name must be at least 3 characters');
        isValid = false;
    }
    ...
    if (!username) {
        ...
        showError('usernameError', '⚠️ Username must be at least 3 characters');
        isValid = false;
    } else if (!/^[a-zA-Z0-9_]+$/.test(username)) {
        showError('usernameError', '⚠️ Only letters, numbers, and underscores allowed');
        isValid = false;
    } else if (usersDatabase.some(u => u.username === username)) {
        showError('usernameError', '⚠️ This username is already taken. Try another one.');
        isValid = false;
    }

    if (!email) {
        showError('registerEmailError', '⚠️ Email address is required');
        isValid = false;
    } else if (!isValidEmail(email)) {
        showError('registerEmailError', '⚠️ Please enter a valid email format');
        isValid = false;
    } else if (usersDatabase.some(u => u.email === email)) {
        showError('registerEmailError', '⚠️ This email is already registered. Try logging in instead.');
        isValid = false;
    }

    if (!phone) {
        showError('phoneError', '⚠️ Phone number is required');
        isValid = false;
    } else if (phone.replace(/\D/g, '').length < 10) {
        showError('phoneError', '⚠️ Please enter a valid phone number (at least 10 digits)');
        isValid = false;
    }

    if (!password) {
        showError('registerPasswordError', '⚠️ Password is required');
        isValid = false;
    } else if (password.length < 8) {
        showError('registerPasswordError', '⚠️ Password must be at least 8 characters');
        isValid = false;
    } else if (!password.match(/[A-Z]/)) {
        showError('registerPasswordError', '⚠️ Password must contain at least one uppercase letter');
        isValid = false;
    } else if (!password.match(/[0-9]/)) {
        showError('registerPasswordError', '⚠️ Password must contain at least one number');
        isValid = false;
    }

    if (!confirmPassword) {
        showError('confirmPasswordError', '⚠️ Please confirm your password');
        isValid = false;
    ...
    if (!terms) {
        showError('termsError', '⚠️ You must accept the terms and conditions to continue');
        isValid = false;
    }
    ...
    setTimeout(() => {
        ...
        showSuccessModal(
            ...
            `Welcome to JD Restaurant, ${fullName}! Your account has been created. You will be redirected to your dashboard.`
        );
